...
- **Atomic runtime claim loop**: the daemon registers local capabilities,
  claims queued TaskRuns, enforces one active TaskRun per issue, honors
  runtime/profile concurrency limits, heartbeats leases, executes work,
  records progress, classifies failures, and retries when policy allows.
