...
| **PR-F** | daemon 后台化（setsid）+ running 孤儿崩溃恢复 + `daemon start/stop/status` | B（独立执行进程）+ C（heartbeat 时间戳可消费） | 收尾：把「派活走开」从 B 甩给用户的前提假设变成产品能力 |

**执行顺序建议**：A 和 B 可并行（都不依赖对方）→ C（依赖 B）→ D（依赖 A+C）、E（依赖 B）、F（依赖 B+C）。
**最优先 PR-A**——它修的是 false `done` 这个最伤的正确性 bug，且完全独立。
**PR-F 是 deep-011 收尾**：B 让 HTTP 请求不再和执行绑死（请求生命周期），F 让 daemon 进程不再和终端绑死、崩了能恢复（进程生命周期）。两件不同的事，F 补齐 `task-state-machine.md:69` 早已定义却没实现的 `runtime_recovery`。详见 `docs/architecture/daemon-lifecycle.md`。
